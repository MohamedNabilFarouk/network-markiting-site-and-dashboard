...
<div class="toolbar" id="kt_toolbar">
    ...
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        ...
        <div class="d-flex align-items-center me-3">
            <!--begin::Title-->
            <h1 class="d-flex align-items-center text-dark fw-bolder my-1 fs-3">@lang('site.categories')
            <!--begin::Separator-->
...
<div class="card rounded mb-5 mb-xl-8 shadow-lg">
    ...
    <div class="card-header rounded border-0 pt-5">
        <h3 class="card-title align-items-start flex-column">
            <span class="card-label fw-bolder fs-3 mb-1">@lang('site.All') @lang('site.categories')</span>
        </h3>
        <div class="card-toolbar">
            <a href="{{route('resturants.create')}}" class="btn btn-sm btn-light-primary">
            ...
            <!--end::Svg Icon-->@lang('site.add') @lang('site.resturant') +</a>
        </div>
    </div>
    <!--end::Header-->
    <!--begin::Body-->
            ...
            <table class="table table-hover align-middle gs-0 gy-4">
                ...
                <thead>
                    <tr class="text-center border-3 fw-bolder text-muted bg-light">
                        <th class="ps-4 min-w-325px rounded-start">@lang('site.resturant')</th>
                        <th class="min-w-125px">@lang('site.maincategory')</th>
                        <th class="min-w-125px">@lang('site.address')</th>
                        <th class="min-w-125px">@lang('site.phone')</th> 
                        <th class="min-w-125px">@lang('site.vendor')</th> 
                ...
                @foreach($resturants as $c)
                    <tr class="text-center border-3 m-auto">
                        ...
                        <td class="px-3"> 
                        @foreach($c->mainCategory as $m)    
                        <a href="{{ route('resturants.edit', $c->id) }}" class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{$m->name_ar ?? $m->name_en }}</a> - <br>
                        @endforeach
                        </td>
                        <td class="px-3">{{$c->address}}</td>
                        <td class="px-3">{{$c->phone1}}<br>{{$c->phone2}}</td>
                        <td class="px-3">{{$c->user->name}}</td>
                         <td class="px-3">
                        @if($c->status == 1)
                        ...
                        @endif
                         <td class="border text-center">
                            ...
                            <a href="{{ route('resturants.edit', $c->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                            ...
                            <form action="{{ route('resturants.destroy', $c->id) }}" method="post" id='delform' style="display: inline-block">
                                @csrf
                                @method('delete')
